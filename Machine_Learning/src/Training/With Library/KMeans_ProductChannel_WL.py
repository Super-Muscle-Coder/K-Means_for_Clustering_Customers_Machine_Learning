"""
================================================================================
K-MEANS CLUSTERING - PRODUCT CHANNEL SEGMENTATION (WITH SKLEARN LIBRARY)
================================================================================

Module: K-Means for Product Channel Customer Segmentation using Scikit-Learn
...
- Post-hoc analysis visualizations
...
# ================================================================================
# PRODUCT CHANNEL K-MEANS ANALYZER (SKLEARN VERSION)
# ================================================================================

class ProductChannelKMeansAnalyzer:
    """Complete K-Means analysis pipeline using sklearn for Product Channel segmentation."""
    ...
    def visualize_clusters_2d(self):
        ...
        ax.set_title(f'K-Means Clustering (K={self.k}, {self.init}) - Store vs Web [Sklearn]', 
                    fontsize=14, fontweight='bold')
        ax.legend(loc='best', fontsize=11)
        ax.grid(True, alpha=0.3)
    ...
    def visualize_posthoc_preferences(self):
        ...
        for cluster_id in range(self.k):
            ...
            ax.bar(x + offset, means, width, label=f'Cluster {cluster_id}', 
        ...
        ax.set_title('Product Preferences per Cluster (Post-Hoc) [Sklearn]', fontsize=14, fontweight='bold')
    ...
    def visualize_dominant_product_distribution(self, top_n=8):
        ...
        for cluster_id in range(self.k):
            ...
            offset = width * cluster_id
            ax.bar(x + offset, pcts, width, label=f'Cluster {cluster_id}', 
                  color=self.colors[cluster_id], alpha=0.85, edgecolor='#333333', linewidth=0.7)
        
        ax.set_xlabel('Dominant Product (Top categories)', fontsize=12, fontweight='bold')
        ax.set_ylabel('Percentage (%)', fontsize=12, fontweight='bold')
        ax.set_title('Dominant Product Distribution per Cluster [Sklearn]', fontsize=14, fontweight='bold')
        ax.set_xticks(x + width * (self.k - 1) / 2)
    ...
    def visualize_top_product_share(self):
        ...
        ax.bar(labels, means, color=self.colors, alpha=0.85, edgecolor='#333333', linewidth=0.7)
        ax.set_ylabel('Mean Top_Product_Share (Scaled)', fontsize=12, fontweight='bold')
        ax.set_title('Top Product Share by Cluster [Sklearn]', fontsize=14, fontweight='bold')
...
def main():
    ...
    OUTPUT_GRAPH_DIR = r"C:\Project\Machine_Learning\Machine_Learning\graph\Training\WIth Library\ProductChannel_WL"
    
    # Configuration: K=4 (adjustable), k-means++ init, Use voting
    analyzer = ProductChannelKMeansAnalyzer(
        input_csv=INPUT_CSV,
        output_graph_dir=OUTPUT_GRAPH_DIR,
        k=4,  # ← Adjust K based on voting suggestions
